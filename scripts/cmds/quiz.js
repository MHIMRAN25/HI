...
module.exports = {
  ...
  onStart: async function ({ api, event, usersData, args }) {
    ...
    try {
      ...
      api.sendMessage(quizMsg, threadID, (err, info) => {
        ...
        if (!global.quizStats[senderID]) global.quizStats[senderID] = { played: 0, won: 0 };
        ...
        // 🟢 1 Minute por silent unsend:
        setTimeout(() => {
          if (sessions.has(senderID)) {
            api.unsendMessage(info.messageID); // Quiz msg delete hobe
            sessions.delete(senderID); // Session clear hobe
          }
        }, 60000); // 60 seconds
  ...
  onReply: async function ({ event, api, Reply, usersData }) {
    ...
    if (senderID !== author) return api.sendMessage("𝑻𝒉𝒊𝒔 𝒊𝒔 𝒏𝒐𝒕 𝒚𝒐𝒖𝒓 𝒒𝒖𝒊𝒛 𝑩𝒂𝒃𝒚 🐸", threadID, messageID);
    
    const session = sessions.get(senderID);
    if (!session) return;

    let userAnswer = body.trim().toLowerCase();
    if (userAnswer.startsWith("answer ")) userAnswer = userAnswer.replace("answer ", "").trim();

    // Reply pawar sathe sathe session clear jate setTimeout r kaj na kore
    sessions.delete(senderID);
    api.unsendMessage(messageID); // User er reply delete

    if (userAnswer === session.correctAnswer) {
      const rewardCoins = 500;
      const rewardExp = 121;
      const userData = await usersData.get(senderID);
      await usersData.set(senderID, { money: (userData.money || 0) + rewardCoins, exp: (userData.exp || 0) + rewardExp });
      if (global.quizStats[senderID]) global.quizStats[senderID].won += 1;
      
      return api.editMessage(`𝑩𝒂𝒃𝒚 𝑪𝒐𝒓𝒓𝒆𝒄𝒕 𝒂𝒏𝒔\n✨ 𝒀𝒐𝒖 𝒘𝒐𝒏 ${rewardCoins} 𝒄𝒐𝒊𝒏𝒔 𝒂𝒏𝒅 ${rewardExp} 𝒆𝒙𝒑`, quizMsgID);
    } else {
      return api.editMessage(`𝑾𝒓𝒐𝒏𝒈 𝒂𝒏𝒔𝒘𝒆𝒓 𝒃𝒂𝒃𝒚 🥹\n📖 𝑪𝒐𝒓𝒓𝒆𝒄𝒕 𝒘𝒂𝒔 ${session.correctAnswer.toUpperCase()}`, quizMsgID);
    }
  }
};
